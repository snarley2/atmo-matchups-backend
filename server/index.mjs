...
// Browser automation scheduler runs in this same Render web service.
// Puppeteer connects to Browserless when BROWSER_PROVIDER=browserless, so
// Render orchestrates the job without running a local Chrome process.
const cronExpression = process.env.DAILY_RUN_CRON || "30 23 * * *";
const cronTimezone = process.env.DAILY_RUN_TIMEZONE || "America/New_York";
let schedulerRunInProgress = false;

console.log(`[scheduler] Cron: ${cronExpression}`);
console.log(`[scheduler] Timezone: ${cronTimezone}`);
...
cron.schedule(
  ...
  async () => {
    ...
    console.log(`[scheduler] Trigger time: ${new Date().toISOString()}`);
    console.log(`[scheduler] schedulerRunInProgress: ${schedulerRunInProgress}`);
    console.log(`[scheduler] run.mjs reports running: ${isDailyAutomationRunning()}`);

    if (schedulerRunInProgress || isDailyAutomationRunning()) {
      console.log("[scheduler] Previous automation run is still active; skipping.");
      console.log("==================================================");
      return;
    }
    ...
    try {
      ...
      console.log("[scheduler] Automation result:", result);
    } catch (error) {
      console.error("[scheduler] Automation failed:", error);
    } finally {
      schedulerRunInProgress = false;
      console.log(`[scheduler] Trigger finished: ${new Date().toISOString()}`);
      console.log("==================================================");
    }
  },
  {
